...
function run() {
  return read(path.join(__dirname, 'test_input.txt'), 'utf8').catch(console.error)
    .then(parseData)
    .then(findRoot)
    .then(balanceWeights)
}
...
function findRoot(data) {
  const flatten = (acc, next) => acc.concat(next)
  const findRootReducer = (acc, [name], i, arr) => {
    let children = arr
      .map(([name, weight, children]) => children)
      .reduce(flatten, [])
    !children.includes(name) && (acc += name)
    return acc
  }
  let root = data
    .reduce(findRootReducer, '')

  return [root, data]
}

function balanceWeights([root, data]) {
  return root
}

// Solution from BOT-Brad on Reddit https://www.reddit.com/r/adventofcode/comments/7i44pg/2017_day_7_solutions/dqy53mn/
